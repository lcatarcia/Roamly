...
| **R40** | **Nessun `SetNull` / `ClientSetNull`.** Nessuna FK del modello usa `DeleteBehavior.SetNull` o `ClientSetNull`: entrambe contano come cascade path ai fini dell'errore 1785 e rompono la dimostrazione di ADR-0004 | test di convenzione (**bloccante**). Formalizza una regola finora solo scritta in `CONTEXT.md` §5.2. <br><br>⚠️ **Corretto il 2026-09-25**: la prima stesura nominava `SetDefault`, che **non esiste** nell'enum `DeleteBehavior` di EF Core (i valori sono `Cascade`, `ClientCascade`, `ClientNoAction`, `ClientSetNull`, `NoAction`, `Restrict`, `SetNull`). Il verificatore implementa `SetNull` + `ClientSetNull`, che è anche la forma di `TESTING.md` §6.2 |
